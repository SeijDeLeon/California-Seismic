...
export default function Lectures({ display=false }) {
  ...
  const Sidebar = <aside className='text-left sticky top-0'>
    <p className='text-lg text-left py-3 text-slate-500'>Lectures</p>
      <ul>
        {lectureData.map((lecture) => <li className={(id===lecture.title.slice(0,2) ? 'bg-sky-100 py-2' : 'text-slate-500 py-2')} key={lecture.title}> <NavLink to={`/lectures/${lecture.title.slice(0,2)}`}>{lecture.title}</NavLink> </li>)}
      </ul>
    </aside>
    ...
    <main className='max-w-screen-2xl m-auto px-4'>
      ...
      <div className='flex h-full'>
        ...
        <section className='sm:w-full  lg:w-8/12 overflow-auto flex flex-col justify-center px-4'>
          ...
          <p className='text-sky-800 font-semibold text-lg text-left max-w-full border-b-2 border-r-0 border-t-0 border-l-0 border pt-4'>Summary</p>
          <div className='max-w-full m-auto bg-amber-50 px-4'>
          ...
          <p className='text-sky-800 font-semibold text-lg text-left max-w-full border-b-2 border-r-0 border-t-0 border-l-0 border pt-4'>Written Lecture</p>
