...
export default function Lectures({ display=false }) {
  ...
  if (display === false) {
    ...
    return (
      <main>
        <h2 className='text-xl font-bold my-8'> Lectures</h2>
        <p className='text-left max-w-3xl m-auto'> Start learning here. We recommend reading a single lecture, then watching the corresponding video. After finishing the lecture & video, try out the practice problems before moving on to the next lecture!</p>
        <section className='my-8'>
          {lectures.map( (lecture, index) =>
            (
              <article key={`LectureDescription${index}`}className='flex flex-col md:flex-row m-auto max-w-7xl px-16 my-4'>
                <div className='w-1/3'>
                  <img className='h-auto' alt='Lecture Graphic' src={lecture.img}/>
                </div>
                <section className='w-2/3 px-2'>
                  <h2 className='text-lg font-medium text-left'>{lecture.title}</h2>
                  <p className='text-base text-left'>{lecture.text}</p>
                </section>
