...
export const useBaseShearState = () => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      const Fv = calculateBaseShearUnits.getFv(parseFloat(inputs.longPeriodSpectralAcceleration), parsedSiteClass);
      const Fa = calculateBaseShearUnits.getFa(parseFloat(inputs.shortPeriodSpectralAcceleration), parsedSiteClass);

      const SMS = calculateBaseShearUnits.getSMS(Fa, parseFloat(inputs.shortPeriodSpectralAcceleration));
      const SM1 = calculateBaseShearUnits.getSM1(Fv, parseFloat(inputs.longPeriodSpectralAcceleration));
      ...
      const Cs = calculateBaseShearUnits.getCs(
        ...
        parseFloat(inputs.R),
        parseFloat(inputs.longPeriodSpectralAcceleration),
        parseFloat(inputs.longPeriodTransitionPeriod || 8.0),
        parsedSiteClass
      );
      ...
      const Fvx = calculateBaseShearUnits.getFvx(Cvx, V);
      const totalHeight = updatedFloors.reduce((sum, f) => sum + f.height, 0);
      const storyVs = Fvx.map((_, i) => Fvx.slice(i).reduce((sum, fx) => sum + fx, 0));
