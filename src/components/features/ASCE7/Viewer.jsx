...
export default function Viewer({ section }) {
    ...
    if (isPicture) {
        ...
        if (pictureCurrentChapter) {
            const subsection = pictureCurrentChapter[subParts[0]]?.subsections?.[section];
            if (subsection) {
                images = subsection.imgs.map(img => `${process.env.PUBLIC_URL}/ASCE7/CH${pictureCurrentChapterNumber}/${img}`);
        ...
        <div>
            ...
            <div className="mt-5 max-h-[70vh] overflow-y-auto">
                {images.map((image, index) => (
                    <img key={index} className="w-7/8 mb-5 mx-auto" src={image} alt={image} />
                ))}
            </div>
        </div>
    )
